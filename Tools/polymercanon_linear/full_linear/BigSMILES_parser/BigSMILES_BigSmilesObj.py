...
from .error import BigSMILESError, BigSMILES_BondInconsistencyError, BigSMILES_StoObjMissingTerminalError, \
...
class BigSMILES(SMILES):
    ...
    def addBigSmilesStoObjAtom(self, res, prevAtom, pos, prevBond):
        ...
        try:
            self.G.nodes[nodeId]['BigSMILES_StoObj'] = BigSMILES_StoObj(res.rawStr, pos=pos,
                                                                        index=self.index + [StoObjIdx],
                                                                        prevBond=prevBond)
        except:
            # errorMsg(self.rawStr,pos,'Error','Inconsistency between bonding descriptor')
    ...
    def parseOne(self, res, level, base_pos, pos, prevBond, prevAtom):
        if prevAtom != None:
                ...
                if 'branch' in res.keys():
                    ...
                    if 'bond' not in res.keys():
                        ...
                        if self.G.nodes[prevAtom]['BigSMILES_Bond'].getS_bond() == 'u':
                            ...
                            if self.G.nodes[prevAtom]['BigSMILES_Bond'].checkConsistency(self.Bond_Dict,
                                                                                         prevAtom) == -1:
                                errorMsg(self.rawStr, base_pos + pos, 'Error',
                                         'Inconsistent bond trailing the bonding descriptor.')
                                raise BigSMILESError

                        if self.G.nodes[prevAtom]['BigSMILES_Bond'].getBondOrder() != self.G.nodes[prevAtom][
                            'BigSMILES_Bond'].getBondOrder(trailing_bond):
                            errorMsg(self.rawStr, base_pos + pos, 'Error',
                                     'Inconsistent bond trailing the bonding descriptor.')
                            raise BigSMILESError
            ...
            if self.G.nodes[prevAtom]['_type'] == 'BigSMILES_StoObj':
                ...
                if self.G.nodes[prevAtom]['BigSMILES_StoObj'].rightEnd == None:
                    errorMsg(self.rawStr, base_pos + pos - 1, 'Error',
                ...
                if 'branch' in res.keys():
                    ...
                    if 'bond' not in res.keys():
                        ...
                        # print(self.G.nodes[prevAtom]['BigSMILES_Bond'].getS_bond())
                        #
                        if self.G.nodes[prevAtom]['BigSMILES_StoObj'].rightEnd.getBondOrder() != self.G.nodes[prevAtom][
                            'BigSMILES_StoObj'].rightEnd.getBondOrder(trailing_bond):
                            errorMsg(self.rawStr, base_pos + pos, 'Error',
        ...
        if 'BigSMILES_Bond' in res.keys():
            ...
            try:
                ...
                errorMsg(self.rawStr, base_pos + pos, 'Error',
                         'In parsing BigSMILES stochastic object [' + str(len(self.StoObj_List) + 1) + ']')
                raise BigSMILESError
                # prevBond = None
            return prevBond, prevAtom
        else:
    ...
    def numNeighAll(self):
        valency = {'C': 4, 'c': 3, 'H': 1, 'O': 2, 'o': 1, 'N': 3, 'n': 2, 'S': 2, 's': 1, 'F': 1, 'Cl': 1, 'Br': 1,
                   'I': 1, 'Si': 4}
...
if __name__ == "__main__":
    ...
    testStr = "CCO{[>][<]CCN[>][<]}CCO{[$][$]C[$],[$]CC[$][$]}CCC"
    Polymer = BigSMILES(testStr)
    for p in Polymer[1]:
        print(p.writeStandard())